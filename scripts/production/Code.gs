...
//   const todayStr = Utilities.formatDate(now, Session.getScriptTimeZone(), "yyyy-MM-dd");
  ...
  //   const phone = e.parameter.authcheck;
  ...
  //   return ContentService.createTextOutput(JSON.stringify({ authorized: data.includes(phone) }))
  //     .setMimeType(ContentService.MimeType.JSON);
  // }
// ✅ Full working version with clear matching explained
if (e.parameter.authcheck) {
  //  let debug = {};  
  let phone = e.parameter.authcheck || '';
  phone = phone.trim();
  // debug.step = 'authcheck';
  // debug.inputPhone = phone;
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Authorized Users");
  const lastRow = sheet.getLastRow();
  // debug.lastRow = lastRow;
  const range = sheet.getRange("A2:A" + lastRow);
  // debug.range = range.getA1Notation();
  const dataRaw = range.getValues();
  const data = dataRaw.flat().map(x => String(x).trim());
  // debug.rawData = dataRaw;
  // debug.cleanedData = data;
  // ✅ Use findIndex to get match + row number in one go
  const foundIndex = data.findIndex(entry => entry.includes(phone));
  const isAuthorized = foundIndex !== -1;
  // debug.authorized = isAuthorized;
  // debug.foundIndex = foundIndex;

  if (isAuthorized) {
    const foundRow = foundIndex + 2; // Because A2:A
    // debug.foundRow = foundRow;
    // ✅ Grab adjacent cols (col 2 = B, col 3 = C)
    ...
    // debug.callerState = callerState;
    

  // Get number of columns used in header row to avoid scanning infinitely
  const maxCols = sheet.getLastColumn();
  let col = 4;
  let found = false;

  // ✅ Try to find the first available login column in this user's row
  while (col <= maxCols) {
  ...
  // ✅ If no reusable login column found, create a new one
  if (!found) {
    col = maxCols + 1;
    ...
    // ✅ Set timestamp: value, format, and center-align
    const range = sheet.getRange(foundRow, col);
    range.setValue(new Date());
    range.setNumberFormat("yyyy-MM-dd HH:mm");  // e.g., 12-Jul-25 14:35
